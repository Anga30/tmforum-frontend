...
export default function Home() {
  ...
  if (account) {
    return <main className="page-shell"><p className="eyebrow">TM Forum Party Management</p><h1>Welcome, {account.email}</h1><p>Your administrator account is active with the {account.role} role.</p><nav className="link-list" aria-label="Administrator actions"><Link href="/parties">Manage parties</Link><Link href="/access-control">Manage access control</Link><Link href="/admin/account">View account details</Link><button onClick={() => void signOut()} type="button">Sign out</button></nav></main>;
  }
